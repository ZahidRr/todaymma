...
<head>
    ...
    <title>Management Dashboard | ConnectMe OSS</title>
...
<body class="bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-200 antialiased min-h-screen flex flex-col transition-colors duration-300 overflow-x-hidden">
    ...
    <main id="main-content" class="flex-1 p-8 max-w-[1600px] mx-auto w-full animate-fade-in opacity-0">
        ...
        <div class="flex justify-between items-end mb-8">
            <div>
                <h2 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Executive Summary 📈</h2>
                <p class="text-slate-500 dark:text-slate-400 mt-2 font-medium">Tinjauan performa bisnis, penagihan, dan status client hari ini.</p>
            </div>
            <div class="hidden sm:flex gap-3">
                <button class="flex items-center gap-2 bg-white dark:bg-slate-800 px-4 py-2 rounded-full border border-slate-200 dark:border-slate-700 text-sm font-semibold text-slate-700 dark:text-slate-200 shadow-sm hover:bg-slate-50 transition-colors">
                    Bulan Ini <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                </button>
            </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <!-- 1. INCOMING INSTALLATION -->
            <div class="stat-card bg-white dark:bg-slate-900 rounded-[24px] p-6 shadow-card border border-blue-100 dark:border-blue-900/50 flex flex-col justify-between transition-colors relative overflow-hidden">
                <div class="absolute -right-6 -top-6 w-24 h-24 bg-blue-50 dark:bg-blue-500/10 rounded-full opacity-50"></div>
                <div class="flex justify-between items-start mb-4 relative z-10">
                    <div class="flex items-center gap-2">
                        <div class="p-2 bg-blue-50 dark:bg-blue-500/10 rounded-full"><svg class="w-5 h-5 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg></div>
                        <h3 class="font-semibold text-slate-700 dark:text-slate-300">Incoming Installs</h3>
                    </div>
                </div>
                <div class="flex items-end gap-3 relative z-10">
                    <span class="text-5xl font-extrabold text-blue-600 dark:text-blue-400">{{ $incomingInstalls ?? 0 }}</span>
                    <span class="text-sm font-bold text-slate-500 dark:text-slate-400 mb-1">Client Menunggu</span>
                </div>
            </div>
            ...
            <div class="stat-card bg-white dark:bg-slate-900 rounded-[24px] p-6 shadow-card border border-slate-200 dark:border-slate-800 flex flex-col justify-between transition-colors">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center gap-2">
                        <div class="p-2 bg-green-50 dark:bg-green-500/10 rounded-full"><svg class="w-5 h-5 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg></div>
                        <h3 class="font-semibold text-slate-700 dark:text-slate-300">Est. Pendapatan</h3>
                    </div>
                </div>
                <div class="flex items-end gap-2">
                    <span class="text-3xl font-extrabold text-slate-900 dark:text-white">Rp{{ number_format($pendapatan ?? 0, 0, ',', '.') }}</span>
                </div>
            </div>

            <!-- 3. TOTAL CLIENT (SEKARANG BISA DI-KLIK) -->
            <a href="{{ route('manager.clients') }}" class="stat-card group cursor-pointer bg-white dark:bg-slate-900 rounded-[24px] p-6 shadow-card border border-slate-200 dark:border-slate-800 flex flex-col justify-between transition-colors hover:border-brand-300 dark:hover:border-brand-700">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center gap-2">
                        <div class="p-2 bg-brand-50 dark:bg-brand-500/10 rounded-full"><svg class="w-5 h-5 text-brand-600 dark:text-brand-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg></div>
                        <h3 class="font-semibold text-slate-700 dark:text-slate-300 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">Total Client</h3>
                    </div>
                    <!-- Icon panah kecil penanda clickable -->
                    <div class="p-1 text-slate-300 dark:text-slate-600 group-hover:text-brand-500 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    </div>
                </div>
                <div class="flex items-end gap-3">
                    <span class="text-5xl font-extrabold text-slate-900 dark:text-white">{{ $totalClient ?? 0 }}</span>
                    <span class="text-sm font-bold text-brand-600 dark:text-brand-400 mb-1 bg-brand-50 dark:bg-brand-500/10 px-2 py-0.5 rounded-md">{{ $clientAktif ?? 0 }} Aktif</span>
                </div>
            </a>
            ...
            <div class="stat-card bg-white dark:bg-slate-900 rounded-[24px] p-6 shadow-card border border-slate-200 dark:border-slate-800 flex flex-col justify-between transition-colors">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center gap-2">
                        <div class="p-2 bg-amber-50 dark:bg-amber-500/10 rounded-full"><svg class="w-5 h-5 text-amber-600 dark:text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg></div>
                        <h3 class="font-semibold text-slate-700 dark:text-slate-300">Isu Billing & Churn</h3>
                    </div>
                </div>
                <div class="flex items-end gap-5">
                    <div>
                        <span class="text-4xl font-extrabold text-amber-500">{{ $clientMenunggak ?? 0 }}</span>
                        <span class="text-[10px] font-bold text-amber-600 dark:text-amber-400 block mt-1">MENUNGGAK</span>
                    </div>
                    <div class="h-10 w-px bg-slate-200 dark:bg-slate-700 mb-1"></div>
                    <div>
                        <span class="text-3xl font-extrabold text-red-500">{{ $clientChurn ?? 0 }}</span>
                        <span class="text-[10px] font-bold text-red-600 dark:text-red-400 block mt-1">CHURN (BLN INI)</span>
                    </div>
                </div>
            </div>
        </div>

        <!-- TABEL DATA & FILTER DROPDOWN -->
        <div class="bg-white dark:bg-slate-900 rounded-[24px] shadow-card border border-slate-200 dark:border-slate-800 p-8 transition-colors">
            ...
            <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-lg"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg></div>
                    <h3 class="text-lg font-bold text-slate-900 dark:text-white">Manajemen & Status Client</h3>
                </div>

                <!-- WRAPPER UNTUK SEARCH BAR DAN ICON FILTER -->
                <div class="flex items-center gap-3 w-full xl:w-auto relative">
                    ...
                    <div class="relative w-full md:w-64">
                        ...
                        <input type="text" id="search-client" placeholder="Cari nama / unit..." class="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-semibold focus:ring-2 focus:ring-brand-500 outline-none transition-all">
                    </div>

                    <!-- DROPDOWN FILTER MENU -->
                    <div class="relative">
                        <!-- Icon Button -->
                        <button onclick="toggleFilterDropdown()" class="flex items-center justify-center w-10 h-10 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 hover:text-brand-600 hover:border-brand-300 transition-colors shadow-sm focus:outline-none" title="Filter Lanjutan">
            ...
            <div class="flex flex-wrap gap-2 mb-6">
                <button onclick="filterTower('All', this)" class="tower-btn active px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">Semua Tower</button>
                <button onclick="filterTower('Alamanda', this)" class="tower-btn px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50">Alamanda</button>
                <button onclick="filterTower('Bougenville', this)" class="tower-btn px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50">Bougenville</button>
                <button onclick="filterTower('Chrysant', this)" class="tower-btn px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50">Chrysant</button>
                <button onclick="filterTower('Dahlia', this)" class="tower-btn px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50">Dahlia</button>
                <button onclick="filterTower('Emerald', this)" class="tower-btn px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50">Emerald</button>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-slate-400 dark:text-slate-500 text-[11px] uppercase tracking-wider font-extrabold border-b border-slate-100 dark:border-slate-800">
                            <th class="pb-4 pl-2">Profil Client & Lokasi</th>
                            <th class="pb-4">Kontak (HP)</th>
                            <th class="pb-4">Paket Langganan</th>
                            <th class="pb-4">Tgl Mulai Aktif</th>
                            <th class="pb-4">Masa Aktif / Due Date</th>
                            <th class="pb-4">Status Billing</th>
                            <th class="pb-4 text-center">Riwayat Tiket</th>
                    ...
                    <tbody id="client-table-body" class="text-sm">
                        @forelse($clients ?? [] as $client)
                            ...
                            <!-- DATA-ATTRIBUTES UNTUK FILTERING & SORTING -->
                            <tr class="client-row border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors {{ $client->status == 'isolated' ? 'opacity-75 hover:opacity-100' : '' }}"
                                data-paket="{{ strtolower($client->package) }}"
                                data-status="{{ strtolower($billingStatusText) }}"
                                data-date="{{ $installDate->timestamp }}">
                                
                                <td class="py-5 pl-2 {{ $client->status == 'terminated' ? 'border-l-4 border-red-500' : '' }}">
                                    <div class="flex items-center gap-4">
                                ...
                                <td class="py-5 font-semibold text-slate-600 dark:text-slate-400">
    ...
    <script>
        ...
        function applyFilters() {
            ...
            rows.forEach(row => {
                ...
                // MENCARI ANGKA "30", "50", "100" DI DATA PAKET
